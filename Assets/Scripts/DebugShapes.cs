...
public class DebugShapes : MonoBehaviour {
    ...
    public static void DrawLine(Vector3 pos, Vector3 end, Color col, float dur = 0f) {
        Debug.DrawLine(pos, end, col, dur);
    }

    public static void DrawRay(Vector3 pos, Vector3 direction, Color col, float dur = 0f) {
        Debug.DrawLine(pos, pos + direction, col, dur);
    }

    public static void DrawRay(Vector3 pos, Vector3 direction, float distance, Color col, float dur = 0f) {
        Debug.DrawLine(pos, pos + direction.normalized * distance, col, dur);
    }

    public static void DrawPoint(Vector3 pos, Color col, float scale, float dur = 0.0f) {
        ...
        Debug.DrawLine(points[0], points[1], col, dur);
        Debug.DrawLine(points[2], points[3], col, dur);
        Debug.DrawLine(points[4], points[5], col, dur);

        Debug.DrawLine(points[0], points[2], col, dur);
        Debug.DrawLine(points[0], points[3], col, dur);
        Debug.DrawLine(points[0], points[4], col, dur);
        Debug.DrawLine(points[0], points[5], col, dur);

        Debug.DrawLine(points[1], points[2], col, dur);
        Debug.DrawLine(points[1], points[3], col, dur);
        Debug.DrawLine(points[1], points[4], col, dur);
        Debug.DrawLine(points[1], points[5], col, dur);

        Debug.DrawLine(points[4], points[2], col, dur);
        Debug.DrawLine(points[4], points[3], col, dur);
        Debug.DrawLine(points[5], points[2], col, dur);
        Debug.DrawLine(points[5], points[3], col, dur);

    }

    public static void DrawCube(Vector3 pos, Color col, Vector3 scale, float dur = 0f) {
    ...
    public static void DrawPlane(Vector3 pos, Vector3 normal, Vector3 tangent, float size, Color col, float dur = 0) {
        ...
        Debug.DrawLine(a, d, col, dur);
        Debug.DrawLine(b, c, col, dur);
        Debug.DrawLine(b, d, col, dur);
        Debug.DrawLine(c, d, col, dur);
    }

    public static void DrawCircle(Vector3 pos, Vector3 normal, float radius, Color col, float dur = 0) {
        ...
        for (int k = 0; k < l - 1; k++) {
            Debug.DrawLine(p[k], p[k + 1], col, dur);
        }
        Debug.DrawLine(p[l - 1], p[0], col, dur);
    }

    public static void DrawCircleSection(Vector3 pos, Vector3 min, Vector3 max, Vector3 normal, float minRadius, float maxRadius, Color col, float dur = 0) {
        ...
        Debug.DrawLine(p[0], P[0], col, dur);
        Debug.DrawLine(p[l - 1], P[l - 1], col, dur);
    }

    public static void DrawSphere(Vector3 pos, float radius, Color col, float dur = 0) {
        ...
        for (int k = 0; k < l; k++) {
            ...
            DrawCircle(horiz[k], Vector3.right, r, col, dur);
            DrawCircle(vert[k], Vector3.up, r, col, dur);
            DrawCircle(Z[k], Vector3.forward, r, col, dur);
        }
    ...
    public static void DrawSphereRay(Vector3 start, Vector3 direction, float distance, float radius, int amount, Color col, float dur = 0) {

        Vector3 endPoint = start + (radius + distance) * direction;
        Vector3 endPointSphereCenter = endPoint - (radius * direction);

        for (int i = 0; i < amount; i++) {
            DrawSphere(Vector3.Lerp(start, endPointSphereCenter, (float)i / (amount - 1)), radius, new Color(col.r, col.g, col.b, 0.5f), dur);
        }
        Debug.DrawLine(start, endPoint, col, dur);
    }

    public static void DrawSphereRay(Vector3 start, Vector3 end, float radius, int amount, Color col, float dur = 0) {
        Vector3 v = end - start;
        DrawSphereRay(start, v.normalized, v.magnitude, radius, amount, col, dur);
    }
}